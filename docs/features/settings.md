...
context_room:
  ...
  sources: [src/context_room.mjs, src/review_authority.mjs, src/context_settings.mjs, src/shared_context.mjs, src/codex_prompt_center.mjs, bin/context-room-remote.mjs, schemas/config.schema.json, schemas/shared-skill-locations.schema.json, schemas/shared-instruction-locations.schema.json, test/e2e/hosted-profiles.spec.mjs, docs/agent-configuration.md, docs/features/codex-prompt-center.md, docs/remote-qm.md]
---
...
Settings belongs only to the local runtime profile. Hosted Hub and Hosted Review never render it, restore it from navigation state, call its APIs, or expose the server's local project configuration and Codex prompt state.

## Categories

- `Project`: Shared Context repositories and the selected local project's connection, followed by Markdown templates, device-wide project priority, and project-owned Home sections and routing.
- `Review and trust`: the agent CLI guide, snoozed reviews, one Documents to review scope, and advanced Git-action protection.
- `Agent environment`: startup instructions, local skill discovery, hooks or automation, Shared Skills, and reviewed instruction collections such as **AGENTS.md** and **CLAUDE.md** with their accepted assignments, provider targets, managed links, and conflicts.
- `Preferences`: theme and reading, Explorer and file behavior, interface sounds, and keyboard shortcuts.
- `Advanced extensions`: entry to the global [Codex Prompt Center](codex-prompt-center.md). The dedicated editor keeps its versioned runtime catalog, conflict handling, and private storage separate from project configuration and review.
...
- Apply the semantic, focus, keyboard, contrast, touch, motion, and dialog
  requirements from [Interface accessibility](../assurance/interface-accessibility.md)
  to every Settings category and disclosure.
...
- In a global Context Room, never present the Hub host's private configuration as project configuration. **Review and trust** shows the real aggregate counts for local review files and shared proposals, then explains that watched documents, Git gates, agent-environment scanners, templates, and Home sections belong to individual projects.
- Keep **Review and trust → Agent CLI guide** available before any project is selected. It leads with the three root commands: `ask`, `edit`, and `capabilities`. The guide explains the one human-owned responsibility—accepting or rejecting each file awaiting review—and the mandatory two-confirmation agent rule: ask once, restate the exact action, scope, and effects after the first yes, ask again, and do nothing without a second separate, unambiguous yes. It states that `edit` creates the shared proposal worktree directly and provides a paste-ready instruction for a coding agent.
- To edit project-owned settings from a global room, select a local project or worktree in Explorer. Its **Review and trust**, **Agent environment**, project templates, and Home organization load directly in the current Settings page and save only to that selected project. Shared-only projects have no local project settings.
...
- Load Shared resources only when **Agent environment** opens. Load Startup environment and Context Health only when their disclosure opens; a failure in one of these secondary surfaces must not block Settings or Explorer.
...
Device interface preferences, Codex prompts, templates, document or hook
