...
- Keep **Review → Agent CLI guide** available before any project is selected. It leads with the compact `context ask` documentation route and reveals specialized capabilities only on demand. A nested command disclosure covers project navigation, consultative reviews, shared proposals, Shared Skills, setup, and maintenance. The guide explains the one human-owned responsibility—accepting or rejecting each file awaiting review—exposes `context-room agent help` and `context-room capabilities --format json`, and provides a paste-ready instruction for a coding agent.
...
- For Shared Instructions, require an explicit Markdown source, target path,
  ...
  Never replace unmanaged instruction files. Hooks are not shared.
...
context-room settings plan --set 'startupContext.projectOnly=true' --project <id> --format json
