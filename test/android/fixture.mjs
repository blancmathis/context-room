...
openNotebook(root, { path: 'docs/Second.crnb', id: 'android-second', title: 'Autre carnet', canWrite: allowed });
...
const owner = http.createServer(async (req, res) => {
  try {
    ...
    } else if (req.method === 'GET' && url.pathname === '/scene') data = readNotebook(root, scene.resourceId);
...
fs.writeFileSync(path.join(base, 'fixture.json'), JSON.stringify({ ownerUrl: `http://127.0.0.1:${owner.address().port}`, projectId, resourceId: scene.resourceId }), { mode: 0o600 });
