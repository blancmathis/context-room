...
export async function exportLisiereSnapshot({ source, output, apply = false, expectedRevision } = {}) {
  ...
  if (manifest.version !== 1 || manifest.mediaType !== 'application/vnd.context-room.lisiere-snapshot+json' || !/^[a-f0-9]{64}$/.test(manifest.revision || ''))
