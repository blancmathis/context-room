...
export function createContextInventoryReaders(overrides = {}) {
  return Object.freeze({
    ...
    listDocuments: (root) => listMemoryFiles(root),
    readDocument: (root, relPath) => readMemoryFile(root, relPath),
    readReviewState: (root) => readDocReviewState(root),
    readGlobalReviewLedger: (root) => readGlobalReviewLedger(root),
    readReviewQueue: (root) => buildAgentReviewQueue(root),
    readDoctor: (root) => buildContextRoomDoctorReport(root),
