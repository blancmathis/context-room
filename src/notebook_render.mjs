...
export function notebookSvg(input, { showOrigins = false } = {}) {
  ...
  const items = document.objects.map(o => {
    ...
    let body;
    if (o.type === 'ink') body = `<polyline points="${o.points.map(p => `${p[0]},${p[1]}`).join(' ')}" ${style}/>`;
    ...
    return `<g data-object-id="${o.id}"${transform}><title>${xml(o.type)} · ${xml(o.createdBy.kind)} · r${o.revision}</title>${body}${showOrigins && o.createdBy.kind === 'agent' ? `<text x="${x}" y="${y - 8}" font-family="sans-serif" font-size="11" fill="#555555">agent</text>` : ''}</g>`;
  }).join('');
