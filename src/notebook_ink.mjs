...
export const notebookInkRadius = (point, width = 2) => Math.max(.05, width) * (.2 + .8 * Math.max(0, Math.min(1, point[2] ?? .5))) / 2;
export function notebookInkOutline(points, width = 2) {
  if (!Array.isArray(points) || !points.length) return [];
  const left = [], right = [];
  for (let i = 0; i < points.length; i++) {
    const p = points[i], a = points[Math.max(0, i - 1)], b = points[Math.min(points.length - 1, i + 1)];
    const dx = b[0] - a[0], dy = b[1] - a[1], length = Math.hypot(dx, dy);
    const nx = length ? -dy / length : 0, ny = length ? dx / length : 1, radius = notebookInkRadius(p, width);
