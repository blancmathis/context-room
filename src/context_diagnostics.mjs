...
const RUNTIME_DOCUMENTATION_REFERENCE_KINDS = new Map([
  [".context-room/README.md", "generated-runtime"],
  [".context-room/review-gate.json", "owner-optional-runtime"],
]);
...
/**
 * Classify stable Context Room runtime paths that documentation may name even
 * when no file currently exists. The agent guide is generated by init, setup,
 * and start; the review gate is created only when the owner selects a policy.
 * Other missing `.context-room` paths remain ordinary broken references.
 */
export function runtimeDocumentationReferenceKind(reference = "") {
  const clean = normalizedSlashPath(String(reference || "").trim().replace(/^<|>$/g, "").replace(/[?#].*$/, ""));
  return RUNTIME_DOCUMENTATION_REFERENCE_KINDS.get(clean) || "";
}
