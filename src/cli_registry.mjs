...
export const CLI_COMMAND_REGISTRY = freeze([
  ...
  define("migrate", { summary: "Preview versioned state migration, a private Lisière export or a working notebook import; apply an exact revision with recoverable backups.", arguments: [...TARGET, option("--plan", { value: "boolean", description: "Preview only; also the default." }), option("--apply", { value: "boolean" }), option("--revision"), option("--inspect-lisiere", { description: "Read a completed recovery snapshot inventory; no project registration or old executable required." }), option("--kind", { description: "Inventory selection: all, projects, boards, drafts, conversations, operations or recordings." }), option("--limit", { description: "Inventory page size, 1–200." }), option("--cursor", { description: "Opaque cursor bound to the original snapshot and inventory selection." }), option("--legacy-recording", { description: "Exact retained PCM name. Explicitly attaches without playback, transcription or sending." }), option("--conversation-id", { description: "Existing exact conversation to receive --legacy-recording, instead of --path." }), option("--label", { description: "Human label for the selected recording." }), option("--cutover-lisiere", { description: "Exact original Mac workspace. Preview then explicitly retire the stopped companion source behind a permanent fence." }), option("--legacy-plist", { description: "Exact original companion launch-agent definition, required for Mac cutover." }), option("--rollback-cutover", { value: "boolean", description: "Preview a no-writer pause preserving all recent work; never restores or restarts Lisiere." }), option("--resume-cutover", { value: "boolean", description: "Preview resuming Context Room only after verification of the original writer fence." }), option("--reconcile-lisiere", { description: "Complete Android snapshot to compare with an explicit canonical Mac snapshot; never replays a legacy queue." }), option("--mac-snapshot", { description: "Canonical Mac recovery snapshot for --reconcile-lisiere." }), option("--recovery-view", { description: "Explicit choice when a tablet cache exists: queue projection or tablet copy. Copies never prove delivery or delete Mac objects." }), option("--legacy-actor", { description: "Exact original tablet actor; required for historical request digest comparisons." }), option("--export-lisiere", { description: "Legacy directory containing workspace.sqlite, or a native Android recovery ZIP; validates and exports private recovery data without starting the old service." }), option("--output", { description: "Private export directory outside the legacy source; required with --export-lisiere." }), option("--recordings", { description: "Optional original Android dictation directory; includes bounded PCM in a version-2 recovery snapshot." }), option("--import-lisiere", { description: "Completed recovery snapshot; restores a Mac notebook or conversation, or a Mac/Android text draft." }), option("--legacy-session", { description: "Exact retained transfer ID inside this project's .context-room/lisiere/sessions; preview its frames or recover one without the old service." }), option("--session-frame", { description: "Explicit transfer frame: source, preview:REVISION or board:REVISION. Requires --legacy-session and an unused --path." }), option("--legacy-board", { description: "Exact source board ID; required for notebook import." }), option("--legacy-draft", { description: "Exact draft selector from the inventory; restores text as a Local editing proposal." }), option("--legacy-conversation", { description: "Exact conversation selector; retains original history privately and links an explicit new scoped continuation. No original task is resumed." }), option("--path", { description: "Writable ordinary document location in the target project; required for import." })], scopes: [...PROJECT_SCOPES, "device"], mutation: "mutating", protocol: "preview-apply", effect: "reversible-local", handlerKey: "migrate", ui: "diagnostic", exposure: "canonical" }),
  ...
  define("hub", { summary: "Start or focus the single global Context Room.", arguments: [option("--root"), option("--port", { value: "integer" }), ...DEVICE_START], scopes: ["device"], mutation: "mutating", handlerKey: "hub.start", ui: "both" }),
  ...
  define("setup", { summary: "Initialize a project, register it, and open it in the global Context Room.", arguments: [option("--root"), option("--title"), option("--allow"), option("--watch"), option("--port", { value: "integer" }), ...DEVICE_START], scopes: ["project", "device"], mutation: "mutating", protocol: "legacy-direct", outputSchema: "text/plain", handlerKey: "setup", ui: "both" }),
  define("start", { summary: "Register and focus a project in the global Context Room.", arguments: [option("--root"), option("--port", { value: "integer" }), ...DEVICE_START], scopes: ["project", "device"], mutation: "mutating", protocol: "legacy-direct", outputSchema: "text/plain", handlerKey: "start", ui: "both" }),
